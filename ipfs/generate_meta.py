...
def generate_file(ipfs_prefix: str, image_name: str, prefix: str, meta: dict):
    ...
    with zipfile.ZipFile(archive) as tree:
        ...
        with tree.open("ZIP/Buildix_attributes.csv") as f:
            ...
            for row in reader:
                ...
                generate_file(ipfs_hidden_path, "hidden.png", "./hidden/meta/", row)
                # generate public meta
                generate_file(ipfs_public_path, row["file_name"], "./public/meta/", row)

if __name__ == "__main__":
    import sys, os
    if len(sys.argv) != 4:
        print(f"usage: {os.path.basename(sys.argv[0])} zipfile ipfs_hidden_path ipfs_public_path")
        sys.exit(1)
    FILENAME = sys.argv[1]
